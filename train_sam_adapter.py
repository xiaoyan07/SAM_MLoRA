...
parser.add_argument('--data_dir', type=str, default='/data1/_backups/lxy_backup/data/split_dg/train_name/',
...
parser.add_argument('--viz_dir', type=str, default='./viz/',
                    help='Path to save weight path')
parser.add_argument('--val_img_dir', type=str, default='/data1/_backups/lxy_backup/data/split_dg/valid_img1530/', help='Path to ')
parser.add_argument('--val_gt_dir', type=str, default='/data1/_backups/lxy_backup/data/split_dg/valid_gt1530/', help='Path to ')
parser.add_argument('--image_size', type=int, default=768, help='image crop size')
...
parser.add_argument('--use_rd_branch', type=bool, default=False, help='whether to use road detail branch')
...
def evaluate(img_dir, gt_dir, viz_dir, model, encoder_global_attn_indexes, img_size=1024):
    ...
    pm = er.metric.PixelMetric(2, logdir=None, class_names=['bg', 'road'])
    for i, name in tqdm(enumerate(val), total=len(val), desc='Evaluation'):
        ...
        n = img.shape[1]
        if 'mass' in viz_dir:
            tem = np.zeros(shape=(1504, 1504, 3))
        elif 'sp' in viz_dir:
            tem = np.zeros(shape=(1312, 1312, 3))
        else:
            tem = np.zeros(shape=(1024, 1024, 3))
        ...
        if 'dg' in viz_dir:
            cv2.imwrite(viz_dir + '/' + name[:-7] + 'mask.png', _mask.astype(np.uint8))
            gt = cv2.imread(os.path.join(gt_dir, name.replace('sat.jpg', 'mask.png')))
        else:
...
def main(args):
    ...
    image_list = filter(lambda x: x.find('jpg') != -1, os.listdir(args.data_dir))
    train_list = list(map(lambda x: x[:-4], image_list))

    dataset_train = ImageFolder(train_list, args.data_dir, size=(768, 768))
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if epoch == 70:
            ...
            lr = update_lr(optimizer, lr, 5.0, factor=True)
            save(model_without_ddp, save_path + '/' + args.name + '_70.th')

        if epoch == 90:
            load(model_without_ddp, save_path + '/' + args.name + '.th')
            lr = update_lr(optimizer, lr, 5.0, factor=True)
            save(model_without_ddp, save_path + '/' + args.name + '_90.th')

        if epoch == 110:
            load(model_without_ddp, save_path + '/' + args.name + '.th')
            lr = update_lr(optimizer, lr, 5.0, factor=True)
            save(model_without_ddp, save_path + '/' + args.name + '_110.th')

        if epoch % 10 == 0:
            tb = evaluate(
                img_dir=args.val_img_dir,
                gt_dir=args.val_gt_dir,
                viz_dir=args.viz_dir + args.name + '_dg',
    ...
    save(model_without_ddp, save_path + '/' + args.name + '_120.th')
